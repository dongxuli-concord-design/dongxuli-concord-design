...
/* or                          PS_SHORT_NAME  ( _WIN32 only ) */
{
  ...
  switch (guise) {
    case FFCSNP: if (filesystem == PS_SHORT_NAME) return nt_short_ffcsnp;
      return ffcsnp;
    case FFCJNL: if (filesystem == PS_SHORT_NAME) return nt_short_ffcjnl;
      return ffcjnl;
    case FFCXMT: if (filesystem == PS_SHORT_NAME) return nt_short_ffcxmt;
      return ffcxmt;
    case FFCSCH: if (filesystem == PS_SHORT_NAME) return nt_short_ffcsch;
      return ffcsch;
    case FFCLNC: if (filesystem == PS_SHORT_NAME) return nt_short_ffclnc;
      return ffclnc;
    case FFCXMO: return ffcxmo;
    case FFCXMP: if (filesystem == PS_SHORT_NAME) return nt_short_ffcxmp;
      return ffcxmp;
    case FFCXMD: if (filesystem == PS_SHORT_NAME) return nt_short_ffcxmd;
      return ffcxmd;
    case FFCDBG: if (filesystem == PS_SHORT_NAME) return nt_short_ffcdbg;
      return ffcdbg;
  }
...
/* or                PS_SHORT_NAME  ( _WIN32 only ) */
{
  ...
  switch (format) {
    case FFBNRY: if (filesystem == PS_SHORT_NAME) return nt_short_ffbnry;
      return ffbnry;
    case FFTEXT: if (filesystem == PS_SHORT_NAME) return nt_short_fftext;
      return fftext;
  }
...
characters
                      (added by the header routines or Parasolid); fputs does not add
any
                  */
...
static void read_from_file(file_p file_ptr, char *buffer, int header,
  ...
  if (header || file_ptr->format == FFTEXT) {
    if (max_buffer_len == 1) {
      ...
      if (value == EOF) {
        *ifail = (feof(file_ptr->stream)
                  ? FR_end_of_file : FR_read_fail);
        trace_print(
...
static void extend_schema_filename(char *filename, int *ifail) {
...
#else
  {
    ...
    if (!p_schema_prefix) {
      p_schema_prefix = new char[strlen(FIXED_SCHEMA_PATH)];
      strcpy(p_schema_prefix, FIXED_SCHEMA_PATH);
    }

    if (p_schema_prefix != NULL
        && strlen(p_schema_prefix) + 1 + strlen(filename) <
            max_namelen) {
      strcpy(extended_filename, p_schema_prefix);
...
static void skip_header(file_p file_ptr, int *ifail) {
  ...
  while (!end_header) {
    /* read from the file */
    read_from_file(file_ptr, buffer, 1, max_header_line, &chars_read,
                   ifail);
    if (*ifail != FR_no_errors) return;
    /***
    ...
    if (strncmp(buffer, g_trailer_start, strlen(g_trailer_start)) ==
        0) {
      /*** this is the end of the header */
      end_header = 1;
    } else if (first_line
        && strncmp(buffer, g_preamble_1, strlen(g_preamble_1)
        ) != 0) {
      /*
...
void GOOPPX(const int *nreals, const double *rvals, const int *nints,
...
void GOCLPX(const int *nreals, const double *rvals, const int *nints,
            const int *ivals, int *ifail) {
  trace_print(">>> GOCLPX\n");
  *ifail = CONTIN;
}
...
void GOPIXL(const int *nreals, const double *rvals, const int *nints,
            const int *ivals, int *ifail) {
  trace_print(">>> GOPIXL\n");
  *ifail = CONTIN;
}
...
void GOOPSG(const int *segtyp, const int *ntags, const int *tags,
...
void GOCLSG(const int *segtyp, const int *ntags, const int *tags,
            const int *ngeom, const double *geom, const int *nlntp,
            const int *lntp, int *ifail) {
  trace_print(">>> GOCLSG\n");
  *ifail = CONTIN;
...
void GOSGMT(const int *segtyp, const int *ntags, const int *tags,
            const int *ngeom, const double *geom, const int *nlntp,
            const int *lntp, int *ifail) {
  trace_print(">>> GOSGMT\n");
  *ifail = CONTIN;
