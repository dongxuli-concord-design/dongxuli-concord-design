...
class XcGsQuaternion {
  ...
  /**
   * @description The current XcGs3dVector is considered row vectors.
   * The XcGs3dMatrix by column-major: matrixEntry[0] is the first column, etc.
  ...
  static #sarabandiSkew({matrixEntry, index}) {
    if (index === 1) {
      ...
      XcGsAssert({assertion: false, message: 'Invalid index'});
    }
  }
   ...
   * @returns {XcGsQuaternion} the rotation quaternion from a rotation matrix
   */
  static fromRotationMatrix({matrix}) {
    ...
    return new XcGsQuaternion({w, x, y, z});
  }

  clone() {
    //TODO
    XcGsAssert({assertion: false, message: 'TODO'});
  }
   ...
   */
  get conjugate() {
    return new XcGsQuaternion({w: this.w, x: -this.x, y: -this.y, z: -this.z});
  }
   ...
   * @returns {XcGsQuaternion} the Hamilton product
   */
  multiply({qRight}) {
    return new XcGsQuaternion({
   ...
   * @returns {XcGsQuaternion} the current quaternion after being normalized
   */
  normalize() {
    const normal = Math.sqrt(this.lengthSquared);
    if (normal < new XcGsPrecision().linearPrecision) {
      return this;
    } else {
      this.w /= normal;
      this.x /= normal;
   ...
   * @returns {XcGsQuaternion} the normalized quaternion of the current quaternion
   */
  get normal() {
    const unitQ = new XcGsQuaternion({w: this.w, x: this.x, y: this.y, z: this.z});
    unitQ.normalize();
    return unitQ;
  }
   ...
   * @returns {XcGsQuaternion} quaternion power
   */
  pow({exponent}) {
    ...
    if (n < XcGsContext.gTol.linearPrecision) {
      return new XcGsQuaternion({w: this.w, x: this.x, y: this.y, z: this.z});
    } else {
      const oldAngle = Math.atan2(n, this.w);
      const newAngle = exponent * oldAngle;
      const axisFactor = Math.sin(newAngle) / n;
      return new XcGsQuaternion({w: Math.cos(newAngle), x: this.x * axisFactor, y: this.y * axisFactor, z: this.z * axisFactor});
    }
  }
   ...
   * @returns {XcGs3dMatrix} a rotation matrix
   */
  get matrix() {
    ...
    return XcGs3dMatrix.fromArray({array});
  }
  ...
  static fromAxisAngle({axisVector, angle}) {
    ...
    return new XcGsQuaternion({w: Math.cos(angle/2), x: v.x, y: v.y, z: v.z});
  }
   ...
   * @returns {XcGsQuaternion} a rotation to rotate the fromVector to the toVector.
   */
  static fromTwoVectors({fromVector, toVector}) {
    let rotation = new XcGsQuaternion();
    ...
    if (vector0.isZeroLength() || vector1.isZeroLength()) {
      ...
      if (innerProduct < (-1 + XcGsContext.gTol.linearPrecision)) {
        ...
        const axis = XcGs3dVector.sharedPerpendicularVector({vector0, vector1});
