class XcGm3dLine extends XcGm3dCurve {
  ...
  get axis() {
    ...
    const pkLineSF = XcGmPK_LINE_sf_t.fromJSON({json: pkReturnValue.line_sf});
  ...
  static create({axis}) {
    const lineSF = XcGmPK_LINE_sf_t.fromXcGm3dAxis({axis});
    const params = {
      line_sf: lineSF.toJSON()
    };
    const {error, pkReturnValue} = XcGmCallPkApi('LINE_create', {params});
    XcGmAssert({assertion: !error, message: `Modeling error: ${error}`});
    const line = XcGmEntity.getObjFromTag({entityTag: pkReturnValue.line});
    return line;
  }
}